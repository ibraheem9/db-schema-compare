...
DB Compare is a lightweight, pure-PHP utility designed to compare the schemas of two MySQL databases. It detects differences in tables, columns, indexes, and foreign keys, and automatically generates the SQL statements required to synchronize them.

This tool features a modern, interactive HTML interface with built-in dark and light mode support, making it easy for developers to review differences and apply fixes.

## Features

- **Schema Comparison**: Compares tables, columns, indexes, and foreign keys between two databases.
- **SQL Fix Generation**: Automatically generates `ALTER TABLE` and `CREATE TABLE` statements to make the schemas identical.
- **Interactive UI**: A clean, responsive interface built with HTML, CSS, and JavaScript.
- **Dark/Light Mode**: Built-in theme toggling for comfortable viewing in any environment.
- **AJAX-Powered**: Performs comparisons asynchronously without page reloads.
- **Zero Dependencies**: Requires only PHP with the PDO MySQL extension. No external libraries needed.
